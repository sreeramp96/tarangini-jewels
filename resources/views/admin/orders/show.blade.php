    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-8">
            <div class="bg-[#0d4837] border border-brand-gold/40 rounded-lg shadow-md overflow-hidden">
                <h2 class="text-xl font-semibold text-gray-200 hero-text p-6 border-b border-brand-gold/20">Items</h2>
                <div class="divide-y divide-brand-gold/20">
                        ...
                        <div class="flex items-center justify-between p-6 hover:bg-[#0d4837]/60">
                            <div class="flex items-center space-x-4">
                                ...
                                <img src="{{ $image ? (Str::startsWith($image->image_path, 'http') ? $image->image_path : asset('images/products/' . $image->image_path)) : asset('images/necklace.jpg') }}"
                                     alt="{{ $item->product->name }}" class="w-16 h-16 object-cover rounded-md border border-brand-gold/20">
                                <div>
            ...
            <div class="bg-[#0d4837] border border-brand-gold/40 rounded-lg shadow-md p-6 text-gray-200">
                <h2 class="text-xl font-semibold hero-text mb-4">Price Summary</h2>
                <div class="space-y-2 border-b border-brand-gold/20 pb-4">
        ...
        <div class="space-y-8">
            <div class="bg-[#0d4837] border border-brand-gold/40 rounded-lg shadow-md p-6">
                ...
                <form action="{{ route('admin.orders.update', $order) }}" method="POST">
                    ...
                    <select id="status" name="status"
                            class="block w-full border-gray-300 focus:border-yellow-500 focus:ring-yellow-500 rounded-md shadow-xs bg-white text-gray-800">
                        <option value="pending" @selected($order->status == 'pending')>Pending</option>
                        <option value="shipped" @selected($order->status == 'shipped')>Shipped</option>
                        <option value="delivered" @selected($order->status == 'delivered')>Delivered</option>
                        <option value="cancelled" @selected($order->status == 'cancelled')>Cancelled</option>
                    </select>
                    <button type="submit" class="w-full btn-gold px-4 py-2 rounded-sm font-medium mt-4">Update</button>
                </form>
            </div>

            <div class="bg-[#0d4837] border border-brand-gold/40 rounded-lg shadow-md p-6 text-gray-300">
