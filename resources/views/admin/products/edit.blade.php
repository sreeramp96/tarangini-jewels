...
@section('content')
    ...
    <div class="bg-[#0d4837] border border-brand-gold/40 p-8 rounded-lg shadow-md max-w-4xl">
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="category" name="category_id"
                            class="block w-full mt-1 border-gray-300 focus:border-yellow-500 focus:ring-yellow-500 rounded-md shadow-xs text-gray-700">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="description" name="description" rows="6"
                            class="block w-full mt-1 border-gray-300 focus:border-yellow-500 focus:ring-yellow-500 rounded-md shadow-xs">{{ old('description', $product->description) }}</textarea>
                    ...
                    <div class="mb-6">
                        <label for="is_featured" class="inline-flex items-center">
                            <input id="is_featured" type="checkbox" name="is_featured" value="1" {{ old('is_featured', $product->is_featured) ? 'checked' : '' }}
                                class="rounded-sm border-gray-300 text-yellow-600 shadow-xs focus:ring-yellow-500">
                        ...
                        <div class="flex flex-wrap gap-4">
                            @forelse($product->images as $image)
                                <div class="relative w-32 h-32">
                                    ...
                                    @endif
                                    <label class="absolute bottom-1 right-1 flex items-center p-1 bg-white/70 rounded-sm">
                                        <input type="checkbox" name="delete_images[]" value="{{ $image->id }}"
                                            class="h-4 w-4 text-red-600 border-gray-300 rounded-sm focus:ring-red-500">
            ...
            <div class="mt-6 flex items-center space-x-4">
                ...
                <a href="{{ route('admin.products.index') }}"
                    class="px-8 py-3 rounded-sm text-white font-medium bg-red-500 glow-hover">
                    Cancel
                </a>
            </div>
        </form>
