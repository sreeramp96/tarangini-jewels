    ...
    <div class="flex justify-between items-center w-full px-6 lg:px-20 py-4">
        ...
        <nav class="hidden md:flex items-center space-x-8 text-md font-medium text-gold-gradient">
            ...
            <div class="relative group" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                <button class="flex items-center text-gold-gradient hover:text-brand-gold transition focus:outline-hidden">
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                    ...
                    class="absolute left-0 mt-0 w-48 bg-[#0a2e2b] border border-brand-gold/30 rounded-lg shadow-lg py-2 z-50 text-gray-200"
                    style="display: none;">
        ...
        <div class="flex items-center space-x-5 lg:space-x-6">
            <form action="{{ route('search') }}" method="GET" class="relative">
                <input type="text" name="query" placeholder="Search" class="w-full px-4 py-1.5 text-sm bg-white/10 border border-brand-gold/30 rounded-full text-gray-200 placeholder-gray-400
                              focus:outline-hidden focus:ring-1 focus:ring-brand-gold
            ...
            @auth
                <div class="relative" x-data="{ open: false }" @click.away="open = false">
                    ...
                    <div x-show="open"
                        class="absolute right-0 mt-2 w-48 bg-[#0a2e2b] border border-brand-gold/30 rounded-lg shadow-lg py-2 z-50 text-gray-200"
