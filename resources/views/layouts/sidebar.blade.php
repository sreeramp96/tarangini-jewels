...
<aside id="sidebar"
       class="fixed flex flex-col mt-0 top-0 px-5 left-0 bg-[#0b3d2e]/90 backdrop-blur-md dark:bg-gray-900 dark:border-gray-800 text-gray-900 h-screen transition-all duration-300 ease-in-out z-99999 border-r border-gray-200"
       x-data="{
        openSubmenus: {},
        init() {
    ...
    <div class="pt-8 pb-7 flex"
        ...
        <a href="/" class="flex items-center gap-2">
            <img x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                 class="dark:hidden" src="/images/logo.png" alt="Logo" width="32" height="32"/>

            <img x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                 class="hidden dark:block" src="/images/logo.png" alt="Logo" width="32" height="32"/>

            <img x-show="!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen"
                 src="/images/logo.png" alt="Logo" width="32" height="32"/>

            <span class="text-2xl font-bold hero-text text-gold-gradient whitespace-nowrap"
        ...
        <nav class="mb-6">
            <div class="flex flex-col gap-4">
                @foreach ($menuGroups as $groupIndex => $menuGroup)
                    <div>
                        <!-- Menu Group Title -->
                        <h2 class="mb-4 text-xs uppercase flex leading-5 text-gold-gradient" :class="(!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen) ?
                        ...
                        <ul class="flex flex-col gap-1">
                            @foreach ($menuGroup['items'] as $itemIndex => $item)
                                <li>
                                                           ...
                                                           :class="isActive('{{ $subItem['path'] }}') ?
                                                            ...
                                                            <span class="flex items-center gap-1 ml-auto">
                                                                ...
                                                                @if (!empty($subItem['pro']))
                                                                    <span :class="isActive('{{ $subItem['path'] }}') ?'menu-dropdown-badge-pro menu-dropdown-badge-pro-active' :
                                    ...
                                    @else
                                        <!-- Simple Menu Item -->
                                        <a href="{{ $item['path'] }}" class="menu-item group" :class="[isActive('{{ $item['path'] }}') ? 'menu-item-active' :'menu-item-inactive',
